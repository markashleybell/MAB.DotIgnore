...
namespace MAB.DotIgnore.Tests
{
    ...
    public class IgnoreListTests
    {
        ...
        {
            var list = new IgnoreList(_basePath + "/loadfromfile.gitignore");

            Assert.IsTrue(list.Rules.Count == 1);
        }

        [Test]
        public void Load_Rules_From_List()
        {
            var list = new IgnoreList(new List<string> { "*.cs" });
        ...
        {
            var list = new IgnoreList(new List<string> { "README1.txt" });

            list.AddRule("README2.txt");
            list.AddRules(new List<string> { "README3.txt", "README4.txt" });
        ...
        {
            var list = new IgnoreList(new List<string> { "README1.txt", "README2.txt", "README3.txt", "README4.txt" });

            list.RemoveRule("README2.txt");

            Assert.IsTrue(list.IsIgnored("README1.txt", true));
            Assert.IsFalse(list.IsIgnored("README2.txt", true));
            Assert.IsTrue(list.IsIgnored("README3.txt", true));
            Assert.IsTrue(list.IsIgnored("README4.txt", true));
        }
        ...
        {
            var list = new IgnoreList(new string[] { "README.txt" });

            list.AddRules(_basePath + "/loadfromfile.gitignore");

            Assert.IsTrue(list.IsIgnored("README.txt", true));
            Assert.IsFalse(list.IsIgnored("test.jpg", true));
            Assert.IsTrue(list.IsIgnored("test.cs", true));
        }
